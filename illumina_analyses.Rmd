...
pcoa.results.min <- pcoa(dist.order.minion)

pcoa.gg.min <- data.frame("PCoA1" = pcoa.results.min$vectors[,1], "PCoA2" = pcoa.results.min$vectors[,2], "Locale" = key.metadata.minion$Locale, "Hour" = key.metadata.minion$Hour)
...
cca(order.taxa ~ Hour, key.metadata.illumina) %>% plot()
